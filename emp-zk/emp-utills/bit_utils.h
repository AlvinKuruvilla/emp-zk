...
namespace emp
{
  ...
  void inner_sort(bool **leaves, vector<bool *> &Left, vector<bool *> &Right, int start, int end, int full_size)
  {
    ...
    if (length > 1)
    {
      ...
      {
        int res = compare(leaves[i], leaves[pivot], 256);
        switch (res)
        {
        case 1:
      ...
      {
        int right_vec_indx = i - (mark + 1);
        leaves[i] = Right[right_vec_indx];
      }
      ...
      */
      ...
      if (mark == 0)
      { // if left was empty
        inner_sort(leaves, Left, Right, start + 1, end, full_size);
      }
      else if (mark == length)
      { // if right was empty
        inner_sort(leaves, Left, Right, start, end - 1, full_size);
      }
      else
      {
        inner_sort(leaves, Left, Right, start, mark - 1, full_size);
        inner_sort(leaves, Left, Right, mark + 1, end, full_size);
      }

      // somtimes works, but mostly leads to infinte loops and core dumps
    }
    else if (length == 1)
    {
      int res_comp = compare(leaves[start], leaves[end], 256);
      bool *tmp = leaves[end];
      switch (res_comp)
      {
      case 3:
        leaves[end] = leaves[start];
        leaves[start] = tmp;
        break;
      default:
        break;
    ...
    {
      cout << "sorting branch has ended" << endl;
    }
  }
